...
GeneratorExport.prototype.generateExportExpressionHtml = function (token) {
  ...
  var defaulted, selected;

  switch (token.text) {
    case 'if':
      _term = _.find(termTempls, {'variable': param1.text});
      value = _parseBoolean(param1.text);

      defaulted = (value === _term.boolean.default) ? 'defaulted' : '';
    ...
    case 'unless':
      ...
      defaulted = (value !== _term.boolean.default) ? 'defaulted' : '';
      selected = (value) ? 'unselected' : 'selected';
    ...
    case 'ifCond':
      ...
      value = _parseIfCond(token);
      defaultValue = _parseIfCondDefault(token);
